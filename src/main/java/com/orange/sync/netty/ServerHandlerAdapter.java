...
import com.orange.sync.service.FrameSyncService;
...
public class ServerHandlerAdapter extends SimpleChannelInboundHandler<Object> {
    ...
    private void handlerWebSocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) {
        ...
        try {
            ...
            //請求分发
            boolean isDispatcher = messageDispatcher.doDispatcher(name, jsonObject.toJSONString(), ctx.channel());
            if (isDispatcher) {
                return;
            }
            //客户端操作帧同步
            FrameSyncService.sync(jsonObject, ctx.channel());
        } catch (Exception e) {
